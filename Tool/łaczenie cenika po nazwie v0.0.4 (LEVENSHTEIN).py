...
# Własna funkcja Levenshteina z wagami
def weighted_levenshtein(s1, s2, subst_weight=1.0, insert_weight=0.5, delete_weight=0.5):
...
# Premia za wspólne słowa
def word_overlap_boost(col1_text, ref_texts, boost_weight=0.1):
    col1_words = set(str(col1_text).lower().split())
    ref_words = set()
...
class LevenshteinApp:
    ...
    def create_widgets(self):
        ...
        tk.Button(self.root, text="Start", command=self.start_thread).grid(row=18, column=0, columnspan=2, pady=10)
        self.progress = ttk.Progressbar(self.root, orient="horizontal", length=400, mode="determinate")
        self.progress.grid(row=19, column=0, columnspan=2, pady=5)
        self.status_label = tk.Label(self.root, text="Status: Oczekiwanie")
        self.status_label.grid(row=20, column=0, columnspan=2)
        self.time_label = tk.Label(self.root, text="")
        self.time_label.grid(row=21, column=0, columnspan=2)
        self.preview_text = tk.Text(self.root, height=5, width=60)
        self.preview_text.grid(row=22, column=0, columnspan=2, pady=5)
    ...
    def process_data(self):
        ...
        for i, row in df.iterrows():
            ...
            for _, comp in df.iterrows():
                ...
                score = s2 * weight2 + s3 * weight3 + s4 * weight4 + bonus + overlap
            ...
            df.at[i, match_col] = round(best_score * 100 / total_weight, 2)
            ...
            self.time_label.config(text=f"Czas: {elapsed:.1f}s | Pozostało: {remaining:.1f}s | {percent*100:.1f}%")
            self.preview_text.delete(1.0, tk.END)
            self.preview_text.insert(tk.END, f"{row[col1]}\n→ {best_text}\nScore: {df.at[i, match_col]}%\nDopasowano z: {best_match}")
            self.progress['value'] = percent * 100
            self.root.update_idletasks()

        out_path = os.path.splitext(self.file_path)[0] + "_WYNIK.xlsx"
