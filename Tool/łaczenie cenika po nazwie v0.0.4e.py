...
class Porownywarka:
    ...
    def porownaj(self):
        ...
        df_filtered = self.df[self.df[kolumna_glowna].notna() & (self.df[kolumna_glowna].astype(str).str.strip() != '')].copy()
        self.progress['maximum'] = len(df_filtered)
        ...
        for idx, (i, val1) in enumerate(df_filtered[kolumna_glowna].items()):
            najlepszy_score = -1
            najlepszy_tekst = ""
            ...
            remaining = (elapsed / (idx + 1)) * (len(df_filtered) - idx - 1)
            self.time_label.config(text=f"{idx+1}/{len(df_filtered)} | Czas: {elapsed:.1f}s | Pozostało: {remaining:.1f}s")
            self.podglad.delete(1.0, tk.END)
            self.podglad.insert(tk.END, f"{val1}\n→ {najlepszy_tekst}\nScore: {najlepszy_score}%\n")
            self.root.update_idletasks()

        self.df[kolumna_docelowa] = self.df[kolumna_docelowa].astype(object)
        self.df[kolumna_procent] = self.df[kolumna_procent].astype(object)

        self.df.loc[df_filtered.index, kolumna_docelowa] = [str(w) for w in wyniki_dopasowania]
        self.df.loc[df_filtered.index, kolumna_procent] = [int(p) for p in wyniki_procentowe]
