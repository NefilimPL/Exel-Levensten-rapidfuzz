...
class RapidFuzzApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Porównanie kolumn - RapidFuzz")
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        self.col3_combo = combo("Kolumna 3:", 7)
        self.col4_combo = combo("Kolumna 4:", 8)

        self.bonus_entry = entry("Premia za wspólne słowa:", 9, "0.1")
        self.min_threshold_entry = entry("Minimalny próg ogólny:", 10, "0.85")

        tk.Button(self.root, text="Start", command=self.start_thread).grid(row=11, column=0, columnspan=2, pady=10)
        ...
        self.time_label.grid(row=14, column=0, columnspan=2)
        self.preview_text = tk.Text(self.root, height=5, width=60)
        self.preview_text.grid(row=15, column=0, columnspan=2, pady=5)
    ...
    def process_data(self):
        ...
        bonus_weight = float(self.bonus_entry.get())
        ...
        valid_rows = df[df[col1].notnull()].copy()
        ...
        for i, (idx, row) in enumerate(valid_rows.iterrows()):
            ...
            for _, comp in df.iterrows():
                ...
                for col in [col2, col3, col4]:
                    try:
                        scores.append(token_set_ratio(str(row[col1]), str(comp[col])) / 100)
                    except:
                        scores.append(0.0)
                raw_score = sum(scores) / len(scores)
                overlap = word_overlap_boost(row[col1], [comp[col2], comp[col3], comp[col4]], boost_weight=bonus_weight)
                total_score = raw_score + overlap
                percent = round(min(100, (raw_score + overlap) * 100), 2)
            ...
            if best_score >= min_score:
                df.at[idx, match_col] = best_percent
                df.at[idx, target_col] = best_text
        ...
        out_path = os.path.splitext(self.file_path)[0] + "_WYNIK_RAPIDFUZZ.xlsx"
        df.to_excel(out_path, index=False)
        messagebox.showinfo("Zakończono", f"Zapisano do: {out_path}")

if __name__ == "__main__":
    root = tk.Tk()
    app = RapidFuzzApp(root)
    root.mainloop()
